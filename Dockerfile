...
# Verify the build
RUN xx-verify --static target/$(xx-cargo --print-target-triple)/release/rstimeapi

# Copy the executable to an easily-findable path
RUN mkdir -p /app/target/release
RUN cp target/$(xx-cargo --print-target-triple)/release/rstimeapi /app/target/release

FROM scratch AS runtime
COPY --from=builder /app/target/release/rstimeapi /rstimeapi
EXPOSE 3000
ENTRYPOINT ["/rstimeapi"]
